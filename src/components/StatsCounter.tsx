...
export default function StatsCounter() {
  const renderIcon = (name: string) => {
    const classStyle = "w-7 h-7 text-[#E5AF2B]";
    switch (name) {
      case "Briefcase":
  ...
  return (
    <section className="bg-[#0F2D63] py-20 relative overflow-hidden">
      ...
      <div className="max-w-[1320px] mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 divide-y md:divide-y-0 lg:divide-x divide-white/10">
          {STATS.map((stat, idx) => (
            <motion.div
              key={idx}
              ...
              className="flex flex-col items-center text-center pt-8 md:pt-0 lg:px-8 first:pt-0"
            >
              {/* Gold Icon with circular border */}
              <div className="mb-5 p-4 rounded-full bg-white/5 border border-white/10 hover:border-[#E5AF2B]/40 hover:bg-white/10 transition-all duration-300">
                {renderIcon(stat.iconName)}
              </div>

              {/* Statistical Value */}
              <CountUpValue value={stat.count} suffix={stat.suffix} />

              {/* Label */}
              <p className="text-white/70 text-sm font-semibold tracking-wide uppercase mt-2">
                {stat.label}
              </p>
            </motion.div>
          ))}
